            ...
            <div class="col-md-12">
                ...
                <div class="card-header p-1">
                    <ul class="nav nav-pills" id="tabMenu">
                        <li class="nav-item">
                            <a class="nav-link active btn-sm py-1 m-1" data-toggle="pill" href="#running">Running
                                ({{ $cabines->where('check_out', '>=', date('Y-m-d'))->count() }})</a>
                        </li>
                        <li class="nav-item">
                                        ...
                                        <tbody>
                                            @foreach ($cabines->where('check_out', '>=', date('Y-m-d')) as $cabin)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <img src="{{ asset('') }}/{{ $cabin->patient->photo ?? 'images/default.jpg' }}"
                                                            class="img-thumbnail" alt="No Image found" width="60">
                                                        <br>
                                                        <span>{!! $cabin->user->name !!}</span>
                                                    </td>
                                                    <td>{{ date('d-M-Y (h:i a)', strtotime($cabin->check_in)) }}</td>
                                                    <td>{{ date('d-M-Y (h:i a)', strtotime($cabin->check_out)) }}</td>
                                                    <td>{{ $cabin->floorId->floorNo->floor }}</td>
                                                    <td>{{ $cabin->room_no }} </td>
                                                    <td>{{ $cabin->rent }}</td>
                                                    <td width="auto">
                                                        <div class="btn-group">
                                                            <a href="{{ url('booking-view', [$cabin->id, 'cabin', $route, 'active']) }}"
                                        ...
                                        <tbody>
                                            @foreach ($cabines->where('check_out', '<', date('Y-m-d')) as $cabin)
                                                <tr>
                                                    ...
                                                    <td width="auto">
                                                        <div class="btn-group">
                                                            <a href="{{ url('booking-view', [$cabin->id, 'cabin', $route, 'inactive']) }}"
                                                                class="btn btn-sm btn-info py-1">View</a>
                                                        </div>
                                                    </td>
                                                </tr>
