      ...
      <div class="col-md-12">      
         ...
         <div class="card-header p-1">
            <ul class="nav nav-pills" id="tabMenu">
               <li class="nav-item">
                  <a class="nav-link active btn-sm py-1 m-1" data-toggle="pill" href="#pending">Pending ({{ $appointmentRequest->where('status', 'pending')->where('appointment_date', '>=', date('Y-m-d'))->count() }})</a>
               </li>
               <li class="nav-item">
							...
							<tbody>
								@foreach($appointmentRequest->where('status', 'pending')->where('appointment_date', '>=', date('Y-m-d')) as $appointment)
									<tr>
										...
										<td>
											<img src="{{$picture = asset($appointment->doctor->photo ?? 'images/default.jpg') }}" class="img-thumbnail" alt="No Image found" width="80" height="80">
											<br>
										...
										<td width="auto">
											<a class="btn btn-sm btn-outline-primary viewModal" data-toggle="modal" data-target="#viewModal"											
