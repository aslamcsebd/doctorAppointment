					...
					<fieldset class="form-group pt-2 mb-1">
						...
						<div class="row justify-content-center">
							...
							<div class="form-group col-auto {{ (isset($room_type) && $room_type == 'ward') ? 'hide' : '' }}" id="cabinStatus">
								<form action="{{ route('booking_search') }}" method="post" data_id="cabinAction" enctype="multipart/form-data" class="needs-validation">
									@csrf
									<div class="row justify-content-center">
                                ...
                                @foreach ($floors as $floor)
                                    @php
                                        $roomCount = $floor->rooms
                                            ->where('room_type', 'cabin')
                                            ->whereIn('room_no', $unBook)
                                            ->count();
                                    @endphp
                                    @if ($roomCount == 0)
                                        @continue
                                    ...
                                    <tr>
                                        ...
                                        @if ($roomCount == 1)
                                            @foreach ($floor->rooms->where('room_type', 'cabin')->whereIn('room_no', $unBook)->sortBy('room') as $room)
												<td>{{$room->name}}</td>
												<td>
                                                    <a href="{{ url('cabin_book', [$check_in, $check_out, $room->id]) }}"
                                                        class="btn btn-sm btn-primary px-4"
                                                        title="Click now for booking">{{ $room->room_no }}</a>
                                                </td>
                                                <td>{{ $room->rent }}</td>
                                            @endforeach
                                        @endif
                                    ...
                                    <tr>
                                        ...
                                        @if ($wardCount == 1)
                                            @foreach ($room->wards->whereIn('id', $unBook)->sortBy('id') as $ward)
                                                <td>
                                                    <a href="{{ url('ward_book', [$check_in, $check_out, $ward->id]) }}"
                                                        class="btn btn-sm btn-primary px-4"
                                                        title="Click now for booking">{{ $ward->ward_no }}</a>
                                                </td>
                                                <td>{{ $room->rent }}</td>
                                            @endforeach
                                        @endif
